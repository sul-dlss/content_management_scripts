...
# this script compiles MODS records coming from druid-named files into one file
# the required wrappers for MODS bulk upload are added
...
Dir.chdir(folder) do
	...
	filenames.each  do | file | 
		...
		if (file =~ /[a-z]{2}[0-9]{3}[a-z]{2}[0-9]{4}\.xml/)
			...
			# add xmlDoc wrapper
			newFile.write("<xmlDoc id=\"descMetadata\" objectId=\"#{druid}\">" + mods.to_xml + "</xmlDoc>")
